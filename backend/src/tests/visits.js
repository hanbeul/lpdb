import chai from 'chai';
import chaiHttp from 'chai-http';
import app from '../main';

chai.use(chaiHttp);
chai.should();

const testPlate = { "plate": "howareyou", "image": ""}

describe("Visits", () => {
    describe("GET /api/visits/", () => {
        //Test to get all visits record
        it("should get all visits record", (done) => {
            chai.request(app)
                .get('/api/visits/')
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a('array');
                    done();
                });
        });
        it("should get a single visit record", (done) => {
            const id = 1;
            chai.request(app)
                .get(`/api/visits/${id}`)
                .end((err, res)=> {
                    res.should.have.status(200);
                    res.body.should.be.a('array');
                    done();
                });
        });
        it("should get a page of visits", (done) => {
            const id = 1;
            chai.request(app)
                .get(`/api/visits/page/${id}`)
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a('array');
                    done();
                });
        });
        it("should get the total visits of a plate", (done) => {
            const id = 1;
            chai.request(app)
                .get(`/api/visits/countvisits/${id}`)
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a('array');
                    done();
                });
        });
        it("should get the total page count", (done) => {
            chai.request(app)
                .get(`/api/visits/pagecount/total`)
                .end((err, res) => {
                    res.should.have.status(200);
                    res.body.should.be.a('object');
                    done();
                });
        });
    })
    describe("POST /api/visits/", () => {
        it("should post a new test visit", (done) => {
            chai.request(app)
                .post('/api/visits/')
                .send(testPlate)
                .end((err, res) => {
                    res.should.have.status(200);
                    done();
                });
        });
    })
})

